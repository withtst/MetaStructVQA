"""Type 2b -> 2b_maskfree (mask_free) conversion script
...
OUTPUT_PATH = os.path.join(BASE_DIR, '../MetaStructVQA_Dataset/QA_pairs/type_2b_maskfree.json')
TEMPLATE_PATH = os.path.join(BASE_DIR, 'question_patterns/2b-identify_true_or_false.json')
MASKFREE_TEMPLATE_PATH = os.path.join(BASE_DIR, 'question_patterns/2b-identify_true_or_false_maskfree.json')
...
def main():
    ...
    for qa in qa_pairs:
        ...
        # Update q_type
        new_qa['q_type'] = '2b_maskfree'
        maskfree.append(new_qa)
    with open(OUTPUT_PATH, 'w', encoding='utf-8') as f:
        json.dump(maskfree, f, ensure_ascii=False, indent=2)
    print(f"Generated {len(maskfree)} 2b_maskfree questions, output: {OUTPUT_PATH}")
    if unmatched:
