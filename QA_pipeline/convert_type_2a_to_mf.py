"""Type 2a -> 2a_maskfree (mask_free) conversion script
...
OUTPUT_PATH = os.path.join(BASE_DIR, '../MetaStructVQA_Dataset/QA_pairs/type_2a_maskfree.json')
TEMPLATE_PATH = os.path.join(BASE_DIR, 'question_patterns/2a-select_the_correct_description.json')
MASKFREE_TEMPLATE_PATH = os.path.join(BASE_DIR, 'question_patterns/2a-select_the_correct_description_maskfree.json')
...
def main():
    ...
    for qa in qa_pairs:
        ...
        # Update q_type
        new_qa['q_type'] = '2a_maskfree'
        maskfree.append(new_qa)
    with open(OUTPUT_PATH, 'w', encoding='utf-8') as f:
        json.dump(maskfree, f, ensure_ascii=False, indent=2)
    print(f"Generated {len(maskfree)} 2a_maskfree questions, output: {OUTPUT_PATH}")
    if unmatched:
